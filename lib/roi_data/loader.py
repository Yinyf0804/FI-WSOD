...
from roi_data.minibatch import get_minibatch, get_minibatch_tuple, get_minibatch_sametuple
import utils.blob as blob_utils
# from model.rpn.bbox_transform import bbox_transform_inv, clip_boxes


class RoiDataLoader(data.Dataset):
    def __init__(self, roidb, num_classes, training=True, sametuple=False):
        self._roidb = roidb
        self._num_classes = num_classes
        self.training = training
        self.sametuple = sametuple
    ...
    def __getitem__(self, index_tuple):
        ...
        if self.sametuple:
            blobs, valid, uni_index, im_scales, inv_index = get_minibatch_sametuple(single_db, self._num_classes)
            blobs['data_t'] = blobs['data_t'].squeeze(axis=0)
        else:
            blobs, valid, uni_index, im_scales, inv_index = get_minibatch(single_db, self._num_classes)
        ...
        blobs['indexes'] = index
        blobs['uni_index'] = uni_index
        blobs['inv_index'] = inv_index
...
def collate_minibatch(list_of_blobs):
    ...
    for blobs in list_of_blobs:
        ...
        elif 'data_t' in blobs:
            lists.append({'data' : blobs.pop('data'),
                      'data_t' : blobs.pop('data_t'),
                      'rois' : blobs.pop('rois'),
                      'labels' : blobs.pop('labels'),
                      'indexes': blobs.pop('indexes'),
                      'roidb': blobs.pop('roidb'),
                      "im_scales": blobs.pop('im_scales'),
                      "uni_index": blobs.pop('uni_index'),
                      "inv_index": blobs.pop('inv_index'),
                      })
        else:
            lists.append({'data' : blobs.pop('data'),
                      'rois' : blobs.pop('rois'),
