...
from modeling import model_builder_oicr_bs3, model_builder_oicr_fiwsod
import nn as mynn
...
def test_net(
    ...
    for i, entry in enumerate(roidb):
        ...
        if (i+1) % Sav_Iter == 0 and 'coco' in dataset_name:
            # det_name = 'discovery_part.pkl' if 'train' in dataset_name else 'detections_part.pkl'
            det_name = 'discovery_part.pth' if 'train' in dataset_name else 'detections_part.pth'
            det_file = os.path.join(output_dir, det_name)
            if i >= Sav_Iter:
                # with open(det_file, 'rb') as f:
                #     all_boxes_part = pickle.load(f)['all_boxes']
                # all_boxes.update(all_boxes_part)
                # save_object(dict(all_boxes=all_boxes), det_file)
                all_boxes_part = torch.load(det_file)['all_boxes']
                all_boxes.update(all_boxes_part)
                torch.save(dict(all_boxes=all_boxes), det_file)
            else:
                # save_object(dict(all_boxes=all_boxes), det_file)
                torch.save(dict(all_boxes=all_boxes), det_file)
            # print(list(all_boxes.keys()))
            print("Boxes has been saved with length {}".format(len(list(all_boxes.keys()))))
            all_boxes = {}

    if i >= Sav_Iter  and 'coco' in dataset_name:
        # with open(det_file, 'rb') as f:
        #     all_boxes_part = pickle.load(f)
        # all_boxes.update(all_boxes_part)
        all_boxes_part = torch.load(det_file)['all_boxes']
        all_boxes.update(all_boxes_part)
    ...
    if 'train' in dataset_name:
        ...
        elif i >= Sav_Iter and 'coco' in dataset_name:
            det_name = 'discovery.pth'
        else:
...
def initialize_model_from_cfg(args, gpu_id=0):
    ...
    if cfg.MODEL.METHOD == "oicr_fiwsod":
        model = model_builder_oicr_fiwsod.Generalized_RCNN()
    else:
        model = model_builder_oicr_bs3.Generalized_RCNN()
    model.eval()
...
def test_net_on_coco(
    ...
    roidb, dataset, start_ind, end_ind, total_num_images = get_roidb_and_dataset(
        dataset_name, proposal_file, ind_range=None
    )
    model = initialize_model_from_cfg(args, gpu_id=gpu_id)
    num_images = len(roidb)
    num_classes = cfg.MODEL.NUM_CLASSES + 1
    ...
    if not os.path.exists(det_file):
        for i, entry in enumerate(roidb):
            ...
            im = cv2.imread(entry['image'])
            cls_boxes_i = im_detect_all(model, im, box_proposals, timers, entry)
            _, _, cls_boxes_i = box_results_with_nms_and_limit(cls_boxes_i['scores'],
                                                            cls_boxes_i['boxes'])
            extend_results(i, final_boxes, cls_boxes_i)
            # all_boxes[entry['image']] = cls_boxes_i
    ...
    results = task_evaluation.evaluate_all(
        dataset, final_boxes, output_dir, test_corloc=False
    )
    return results